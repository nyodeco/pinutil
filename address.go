...
func NewAddressPubKey(serializedPubKey []byte, net *chaincfg.Params) (*AddressPubKey, error) {
	pubKey, err := pinec.ParsePubKey(serializedPubKey, pinec.S256())
	if err != nil {
		return nil, err
	}

	// Set the format of the pubkey.  This probably should be returned
	// from pinec, but do it here to avoid API churn.  We already know the
...
func (a *AddressPubKey) PubKey() *pinec.PublicKey {
	return a.pubKey
}
