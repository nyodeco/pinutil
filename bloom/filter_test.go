...
import (
	...
	"github.com/nyodeco/pinutil/bloom"
)
...
func TestFilterInsert(t *testing.T) {
	...
	err = f.MsgFilterLoad().PinEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsert PinDecode failed: %v\n", err)
		return
	}
...
func TestFilterFPRange(t *testing.T) {
	...
	for _, test := range tests {
		...
		err = f.MsgFilterLoad().PinEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
		if err != nil {
			t.Errorf("PinDecode unexpected error: %v\n", err)
			continue
		}
		if !bytes.Equal(got.Bytes(), want) {
...
func TestFilterInsertWithTweak(t *testing.T) {
	...
	err = f.MsgFilterLoad().PinEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsertWithTweak PinDecode failed: %v\n", err)
		return
	}
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := pinutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(pinutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := pinutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := pinutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := pinutil.NewBlockFromBytes(blockBytes)
	if err != nil {
