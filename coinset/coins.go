...
func (cs *CoinSet) TotalValue() (value pinutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue pinutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue pinutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount pinutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue pinutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue pinutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue pinutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue pinutil.Amount, coins []Coin) (Coins, error) {
...
// pinutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() pinutil.Amount {
	return pinutil.Amount(c.txOut().Value)
}
